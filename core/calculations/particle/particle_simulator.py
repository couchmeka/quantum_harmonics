...
class ParticleSimulator:
    ...
    def apply_quantum_data(self, quantum_data):
        ...
        fidelity = quantum_data["quantum_metrics"]["fidelity"]

        # Reset particle count and initialize new arrays
        self.num_particles = 1024  # Keep fixed size instead of scaling
        self.reset()  # This creates new arrays with correct size

        try:
            if self.use_mps:
                # Make sure arrays match particle count
                phase_field = torch.tensor(phases, device=self.device)
                phase_field = phase_field.repeat(
                    (self.num_particles + len(phases) - 1) // len(phases)
                )[: self.num_particles]

                freq_field = torch.tensor(frequencies, device=self.device)
                freq_field = freq_field.repeat(
                    (self.num_particles + len(frequencies) - 1) // len(frequencies)
                )[: self.num_particles]

                # Apply quantum influences
                self.positions = self.positions * (1 + 0.2 * phase_field.unsqueeze(1))
                self.velocities = self.velocities * (1 + 0.1 * freq_field.unsqueeze(1))

                # Add Pythagorean harmonic influence
                harmonic_values = [p["harmonic_influence"] for p in pythagorean]
                harmonic_field = torch.tensor(harmonic_values, device=self.device)
                harmonic_field = harmonic_field.repeat(
                ...
                self.positions = self.positions * (1 + 0.2 * phase_field[:, np.newaxis])
                self.velocities = self.velocities * (
                    1 + 0.1 * freq_field[:, np.newaxis]
                )
