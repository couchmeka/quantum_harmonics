    ...
    def plot_results(self, results):
        ...
        if solution.size > 0:
            ...
            sm = plt.cm.ScalarMappable(
                cmap="viridis",
                norm=plt.Normalize(
                    vmin=solution.min(), vmax=max(solution.max(), 1e-10)
                ),
            )
            sm.set_array([])
            ...
            ax1.text(0.5, 0.5, "No velocity data available", ha="center", va="center")
        ...
        if solution.size > 0:
            ...
            ax2.text(0.5, 0.5, "No pressure data available", ha="center", va="center")

        # Energy Spectrum
        ax3 = self.figure.add_subplot(gs[0, 2])
        if solution.size > 0:
            v_mag = np.sqrt(np.sum(solution[:, 0:3] ** 2, axis=1))
            v_mag = np.maximum(v_mag, 1e-10)  # Ensure positive values
            ax3.plot(results["t"], v_mag, color="#0099ff")
            ax3.set_yscale("log")  # Use set_yscale instead of semilogy
            ...
            ax3.text(
                0.5,
                0.5,
                "No velocity magnitude data available",
                ha="center",
                va="center",
            )
        ...
        try:
            # Convert frequencies to numpy arrays and ensure they're 1D
            musical_freqs = np.array(
                results.get("original_frequencies", []), dtype=float
            ).ravel()
            fibonacci_freqs = np.array(
                results.get("fibonacci_frequencies", []), dtype=float
            ).ravel()

            print(f"Debug - Frequency arrays:")
            print(f"Musical frequencies: {musical_freqs}")
            print(f"Fibonacci frequencies: {fibonacci_freqs}")

            has_valid_data = False

            if musical_freqs.size > 0 or fibonacci_freqs.size > 0:
                # Filter out zeros and very small values
                if musical_freqs.size > 0:
                    musical_freqs = musical_freqs[musical_freqs > 1e-10]
                    if musical_freqs.size > 0:  # Check again after filtering
                ...
                if fibonacci_freqs.size > 0:
                    # Filter out zeros and very small values
                    fibonacci_freqs = fibonacci_freqs[fibonacci_freqs > 1e-10]
                    if fibonacci_freqs.size > 0:  # Check again after filtering
                        ax4.scatter(
                            fibonacci_freqs,
                            [4] * fibonacci_freqs.size,
                            label="Fibonacci (Hz)",
                            color="orange",
                        )
                        has_valid_data = True
                        print(
                            f"Debug - Plotting {fibonacci_freqs.size} Fibonacci frequencies"
                        )
                ...
                if has_valid_data:
                    ...
                    ax4.text(
                        0.5, 0.5, "No valid frequency data", ha="center", va="center"
                    )
            else:
                ax4.text(
                    0.5, 0.5, "No frequency data available", ha="center", va="center"
                )

        except Exception as e:
            print(f"Error plotting frequencies: {str(e)}")
            ax4.text(
                0.5,
                0.5,
                f"Error plotting frequencies: {str(e)}",
                ha="center",
                va="center",
            )
        ...
        if fibonacci_freqs.size > 0:
            ...
            if fibonacci_freqs.size > 0:  # Check again after filtering
                ...
                has_valid_data = True
                print(f"Debug - Plotting {fibonacci_freqs.size} Fibonacci frequencies")
        ...
        if has_valid_data:
            ...
            ax4.text(0.5, 0.5, "No valid frequency data", ha="center", va="center")
